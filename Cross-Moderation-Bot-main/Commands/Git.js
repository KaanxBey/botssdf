...
exports.run = async(client, message, args) => {
    ...
    message.channel.send(embed.setDescription(`${victim} - ${message.author} Adlı kullanıcı ses kanalına gelmek istiyor kabul ediyor musun ?`)).then(async(msg) => {
        ...
        const onayemoji = (reaction, user) => reaction.emoji.id === "841068537195528212" && user.id === victim.id;
        const redemoji = (reaction, user) => reaction.emoji.id === "841068407079043172" && user.id === victim.id;
